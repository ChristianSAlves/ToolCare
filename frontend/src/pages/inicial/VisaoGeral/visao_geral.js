...
const VisaoGeral = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const responseFuncionarios = await axios.get(
        ...
        const responseFerramentas = await axios.get(
        ...
        const responseEmprestimos = await axios.get(
        ...
        const emprestimosDetalhados = await Promise.all(
          emprestimosData
            .filter(
              (emprestimo) =>
                emprestimo.matriculaFuncionario &&
                isValidUrl(emprestimo.matriculaFuncionario)
            )
            .map(async (emprestimo) => {
              ...
              return {
                ...emprestimo,
                matriculaFuncionario: response.data.matriculaFuncionario,
              };
            })
        );
        ...
        const semEmprestimo = funcionariosData.filter(
          (f) =>
            !emprestimosDetalhados.some(
              (e) => e.matriculaFuncionario === f.matriculaFuncionario
            )
        ).length;
        const comEmprestimo = funcionariosData.length - semEmprestimo;

        const ferramentasDisponiveis = ferramentasData.filter(
          (f) => f.status === "Disponível"
        ).length;
        const ferramentasEmprestadas = ferramentasData.filter(
          (f) => f.status === "Emprestada"
        ).length;
        const ferramentasManutencao = ferramentasData.filter(
          (f) => f.status === "Manutenção"
        ).length;
        const ferramentasPerdidas = ferramentasData.filter(
          (f) => f.status === "Perdida"
        ).length;
    ...
    <div id={styles.tela}>
      ...
      <div className={styles.chartWrapper}>
        ...
        <div id={styles.funcionarios}>
          ...
          <p className={styles.funcionarios}>
            funcionários
            <br />
            cadastrados
          </p>
      ...
      <div id={styles.info_visao_geral}>
        <div id={styles.info_funcionarios}>
          <div className={styles.info_row}>
            <p className={styles.porcentagem}>
              {((dadosVisaoGeral.semEmprestimo / funcionarios) * 100).toFixed(
                0
              )}
              %
            </p>
            <p className={styles.texto}>
              dos funcionários não têm
              <br />
              nenhum empréstimo
            </p>
          </div>
          ...
          <div className={styles.info_row}>
            <p className={styles.porcentagem}>
              {((dadosVisaoGeral.comEmprestimo / funcionarios) * 100).toFixed(
                0
              )}
              %
            </p>
            <p className={styles.texto}>
              dos funcionários têm ao
              <br />
              menos um empréstimo
            </p>
          </div>
        ...
        <div id={styles.info_ferramentas}>
          <div className={styles.info_row}>
            <p className={styles.porcentagem}>
              {(
                (dadosVisaoGeral.ferramentasDisponiveis / ferramentas) *
                100
              ).toFixed(0)}
              %
            </p>
          ...
          <div className={styles.info_row}>
            <p className={styles.porcentagem}>
              {(
                (dadosVisaoGeral.ferramentasEmprestadas / ferramentas) *
                100
              ).toFixed(0)}
              %
            </p>
